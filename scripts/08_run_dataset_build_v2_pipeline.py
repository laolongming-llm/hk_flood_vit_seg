...
2) 顺序执行 scripts/05、scripts/06、scripts/07；
3) 统一输出每一步命令，便于复现与排错。
"""
...
DEFAULT_DRYRUN_CFG = PROJECT_ROOT / "configs" / "dataset_build_v2" / "dry_run_candidate_a.yaml"
DEFAULT_EXPORT_CFG = PROJECT_ROOT / "configs" / "dataset_build_v2" / "export_from_candidate_a.yaml"
...
def main() -> None:
    ...
    dryrun_script = PROJECT_ROOT / "scripts" / "05_vit_dataset_dry_run.py"
    export_script = PROJECT_ROOT / "scripts" / "06_export_vit_dataset_tiles.py"
    balance_script = PROJECT_ROOT / "scripts" / "07_build_train_manifest_balanced.py"
