...
# ===== 可配置项（规则词表）=====
# wetland 子类型中单独归入红树林类（LUM_ID=10）的集合
WETLAND_TO_MANGROVE_SUBTYPES: Final[tuple[str, ...]] = ("mangrove",)
# wetland 子类型中归入自然地（LUM_ID=9）的集合（不含红树林）
...
def mark_non_leaf_participation(conn: sqlite3.Connection) -> int:
    ...
    strong_mangrove_non_leaf = (
        "COALESCE(LUM_ID,255)=10 "
        "AND COALESCE(rule_level,'')='tag_direct' "
        "AND COALESCE(confidence,'')='high' "
        "AND ("
        "COALESCE(natural,'')='mangrove' "
        f"OR (COALESCE(natural,'')='wetland' AND ({wetland_mangrove_cond}))"
        ")"
    )
...
def run_rules(conn: sqlite3.Connection) -> list[tuple[str, int]]:
    ...
    # 新增严格鱼塘规则：other_tags 仅包含 "water"=>"pond"
    ...
    # 既有鱼塘规则保留：natural=water 且 other_tags 含 water=pond（可带 aquaculture=yes）
    fish_pond_cond = (
    ...
    # 人工确认特例：fid=64864 强制归类为 water_body(LUM_ID=8)
    stats.append(
        (
